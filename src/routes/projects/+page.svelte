					...
					<div class="bg-[#1e1e1e] border border-[#2b2d31] rounded-xl overflow-hidden">
						...
						<ul class="divide-y divide-[#2b2d31]">
							{#each getClientProjects(client.id) as project}
								...
								{:else}
									<li class="flex items-center justify-between group p-3 px-4">
										...
										<div class="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-none">
											<button aria-label="Edit project" onclick={() => startEditProject(project)} class="text-[#858585] hover:text-[#cccccc] p-1">
												<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
													<path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" />
												</svg>
											</button>
											<button aria-label="Delete project" onclick={() => deleteProject(project.id)} class="text-[#858585] hover:text-[#f48771] p-1">
												<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
								...
								{/if}
							{:else}
								<li class="text-xs text-[#858585] p-3 px-4 italic">No projects for this client.</li>
							{/each}
						</ul>
					</div>
				{/each}
